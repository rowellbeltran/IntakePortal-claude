/**
 * UiPath Data Fabric Seeder — Continuation
 *
 * FMIPatient (18) and FMIPatientVisit (18) are already seeded.
 * This script:
 *   1. Fetches existing FMIPatient IDs to rebuild the MRN→ID map
 *   2. Deletes the 8 partial FMIReconciliationItem records
 *   3. Seeds: FMIReconciliationItem, FMICareGap, FMIScreeningLab,
 *             FMIROI, FMINurseSummary, FMITodayVisit, FMIWeeklyMetric
 *
 * Usage:
 *   node seed-continue.mjs
 */
...
async function queryAll(entityName) {
  const url = `${BASE}/${entityName}/query`;
  const res = await fetch(url, {
    method: "POST",
    headers: headers(),
  ...
  const body = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(`Query ${entityName} failed: ${res.status} ${JSON.stringify(body)}`);
  return body.value || [];
}

async function deleteRecord(entityName, id) {
  const url = `${BASE}/${entityName}/delete`;
  const res = await fetch(url, {
    method: "POST",
    headers: headers(),
...
async function deleteAll(entityName) {
  const records = await queryAll(entityName);
  if (records.length === 0) { console.log(`  ${entityName}: nothing to delete`); return; }
  console.log(`  Deleting ${records.length} existing ${entityName} records...`);
  let deleted = 0;
  for (const r of records) {
    if (await deleteRecord(entityName, r.Id)) deleted++;
    else console.error(`  ✗ Failed to delete ${r.Id}`);
  }
...
async function main() {
  console.log("=== UiPath Data Fabric Seeder (Continuation) ===");
  console.log(`Tenant: ${auth.org}/${auth.tenant}`);

  // Step 1: Fetch existing FMIPatient records to rebuild MRN→ID map
  console.log("\n[Step 1] Fetching existing FMIPatient records...");
  const existingPatients = await queryAll("FMIPatient");
  console.log(`  Found ${existingPatients.length} patient records`);

  const mrnToId = {};
  for (const p of existingPatients) {
    if (p.MRN && p.Id) mrnToId[p.MRN] = p.Id;
  }
  console.log(`  MRN→ID map: ${Object.keys(mrnToId).length} entries`);
  console.log("  Sample:", Object.entries(mrnToId).slice(0, 3).map(([k,v]) => `${k}→${v.slice(0,8)}...`).join(", "));

  if (Object.keys(mrnToId).length === 0) {
    console.error("  ERROR: No patient IDs found. Cannot seed related records.");
    process.exit(1);
  }

  // Step 2: Clean up partial FMIReconciliationItem records
  console.log("\n[Step 2] Cleaning up partial records...");
  await deleteAll("FMIReconciliationItem");

  // Step 3: Build all related records
  console.log("\n[Step 3] Building related records...");
  const reconciliations = [], careGaps = [], screeningLabs = [], rois = [];
  const nurseSummaries = [], complaints = [];

  for (const [mrn, detail] of Object.entries(patientDetails)) {
    const patientId = mrnToId[mrn];
    if (!patientId) {
      console.warn(`  WARNING: No ID for ${mrn} — skipping`);
      continue;
    }
  ...
  console.log(`  Reconciliation items: ${reconciliations.length}`);
  console.log(`  Care gaps: ${careGaps.length}`);
  console.log(`  Screenings/Labs: ${screeningLabs.length}`);
  console.log(`  ROIs: ${rois.length}`);
  console.log(`  Nurse summaries: ${nurseSummaries.length}`);
  console.log(`  Complaints: ${complaints.length}`);

  // Step 4: Seed all remaining entities
  ...
  // FMIPatientComplaint — uncomment once entity is confirmed in Data Fabric
  ...
  console.log("\n\n=== Continuation seeding complete! ===");
  console.log("\nFinal counts (verify in Data Fabric UI):");
  console.log("  FMIPatient: 18 (unchanged)");
  console.log("  FMIPatientVisit: 18 (unchanged)");
  console.log(`  FMIReconciliationItem: ${reconciliations.length}`);
  console.log(`  FMICareGap: ${careGaps.length}`);
  console.log(`  FMIScreeningLab: ${screeningLabs.length}`);
  console.log(`  FMIROI: ${rois.length}`);
  console.log(`  FMINurseSummary: ${nurseSummaries.length}`);
  console.log(`  FMITodayVisit: ${visitsToday.length}`);
  console.log(`  FMIWeeklyMetric: ${weeklyMetrics.length}`);
}
